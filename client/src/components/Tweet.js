...
function Tweet(props) {
  ...
  const handleCommentSubmit = (e) => {
    ...
    const comment = {
      ...
      commentId: moment().valueOf().toString(), // Ensure commentId is a string and unique
    };
  ...
  // Generate a unique ID for use as a key if needed
  const uniqueId = `tweet-${tweetId || Math.random().toString(36).substr(2, 9)}`;
        ...
        <div className="post">
          ...
          {loading ? (
            ...
            comments.map((comment, index) => (
              <Comment
                key={comment.commentId || `comment-${index}-${uniqueId}`}
